...
inline void insert_arg(std::vector<std::pair<std::string::size_type, std::string::size_type>> &pairs,
					   std::uint32_t &str_id, std::uint32_t &pair_id, const std::string &str, std::stringstream &stream)
{
}

template <typename T>
inline void insert_arg(std::vector<std::pair<std::string::size_type, std::string::size_type>> &pairs,
					   std::uint32_t &str_id, std::uint32_t &pair_id, const std::string &str, std::stringstream &stream, T first)
{
...
inline void insert_arg(std::vector<std::pair<std::string::size_type, std::string::size_type>> &pairs,
					   std::uint32_t &str_id, std::uint32_t &pair_id, const std::string &str, std::stringstream &stream, T first, Args... args)
{
	if (pair_id < pairs.size())
	{
		if (str_id < pairs[pair_id].first)
